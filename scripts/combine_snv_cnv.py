        ...
        for line in f:
            ...
            if partners == "No-String":
                ...
                if main not in genes:
                    genes.append(main)
                gene_groups[main] = _unique_preserve_order(genes)
    return gene_groups
...
def merge_with_cnv(snv_df, cnv_path, cnv_sample_regex: str | None):
    ...
    def _join_unique(vals):
        uniq = sorted({str(v).strip() for v in vals if pd.notna(v) and str(v).strip() and str(v).strip() != "."})
        return ",".join(uniq) if uniq else None

    # Collapse duplicate SNV keys (defensive against malformed inputs / duplicated gene list entries)
    snv_df = (
        snv_df.groupby(["MainGene", "GeneGroup", "Gene", "Sample"], dropna=False, as_index=False)
        .agg({
            "SNV_EventTypes": _join_unique,
            "Protein_Changes": _join_unique
        })
    )

    # Collapse duplicate CNV keys to avoid Cartesian expansion on merge
    cnv_df = (
        cnv_df.groupby(["MainGene", "GeneGroup", "Gene", "Sample"], dropna=False, as_index=False)
        .agg({"CNV_EventType": _join_unique})
    )
